...
data "azurerm_public_ip" "get_my_public_ip" {
  name                = azurerm_public_ip.my-tf-public-ip.name
  resource_group_name = azurerm_virtual_machine.my-tf-instance.resource_group_name
}
output "My_ip_for_ssh" {
  value = data.azurerm_public_ip.get_my_public_ip.ip_address
}
...
resource "null_resource" "remote" {
  provisioner "remote-exec" {
    connection {
      type        = "ssh"
      host        = data.azurerm_public_ip.get_my_public_ip.ip_address
      port        = 22
      user        = var.azure_linux_user_name
      host_key    = var.azure_ssh_pub_key
